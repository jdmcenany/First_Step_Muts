...
generateOrganisms, generateOrganismsDirichlet, generateOrganismsNoTradeoffs, generateOrganismsSpecialists, generateOrganismsTikhonov, generateOrganismsWideR0: Functions for randomly sampling a set of organisms from a common statistical distribution. We use (1) a sampling procedure with binary resource usage, (2) Dirichlet-distributed resource usage, (3) binary resource usage without metabolic tradeoffs, (4) single resource usage where each species has its own resource, (5) binary resource usage with energy costs ("pure fitnesses") drawn according to the procedure in Tikhonov & Monasson 2017, and (6) binary resource usage where the number of resources used by each species follows a uniform distribution.
