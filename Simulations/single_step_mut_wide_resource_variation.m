function out = single_step_mut_wide_resource_variation(task_id, N_runs, alpha, epsilon, R0, R,id_out)
% Performs multiple trials of single-step knockout mutations from random
% communities, with exponentially distributed resource supplies
% alpha = S/R
...
fname = "Results/sstep_wide_resource_var_alpha_" + string(alpha) + "_eps_" + string(epsilon) + "_R0_" + string(R0) + "_R_" + string(R) + "_" + string(task_id - id_out) + ".mat";
