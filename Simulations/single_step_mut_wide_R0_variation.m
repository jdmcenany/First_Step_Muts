function out = single_step_mut_wide_R0_variation(task_id, N_runs, alpha, epsilon, R0_min, R0_max, R,id_out)
% Performs multiple trials of single-step knockout mutations from random
% communities, with the number of resources used by each species drawn
% from ~Uniform(R0_min, R0_max)
% alpha = S/R
...
for i = 1:N_runs
    ...
    deltas{i,1} = delta;
    R0s{i,1} = sum(prop.enzymesInSpecies,2);
    ...
    deltas{i,2} = delta;i
    R0s{i,2} = sum(prop.enzymesInSpecies,2);
end

fname = "Results/sstep_wide_R0_variation_alpha_" + string(alpha) + "_eps_" + string(epsilon) + "_R0_" + string(R0_min) + "_to_" + string(R0_max) + "_R_" + string(R) + "_" + string(task_id - id_out) + ".mat";
